                      ...
                      <tbody>
                        ...
                        <tr>
                          <td class="-text-left">
                            ...
                            @for($i=1; $i <= 4; $i++)
                            @if($i <= 2 || ($i >= 3 && $student->exam_type=='written'))
                            <div class="radiobtn @if($question->true_answer==$i) success @endif @if(!$question->answer->status && $question->answer->answer == $i) danger @endif">
                                <input type="radio" class="custom-control-input" @if($question->true_answer==$i) checked @endif disabled id="radio-{{$question->id}}-{{$i}}" name="answer[{{$question->id}}]" value="{{$i}}">
                                <label for="radio-{{$question->id}}-{{$i}}">{{object_get($question, 'answer'.$i) }} </label>
                            </div>
                            @endif
                            @endfor
                            {{-- <div class="radiobtn">
                                <input type="radio" class="custom-control-input" disabled id="radio-{{$question->id}}-2" name="answer[{{$question->id}}]" value="2">
                                <label for="radio-{{$question->id}}-2">{{$question->answer2}}</label>
                            </div>
                        
                            @if($student->exam_type=='written')
                            <div class="radiobtn">
                                <input type="radio" class="custom-control-input" disabled id="radio-{{$question->id}}-3" name="answer[{{$question->id}}]" value="3">
                                <label for="radio-{{$question->id}}-3">{{$question->answer3}}</label>
                            </div>
                        
                            <div class="radiobtn">
                                <input type="radio" class="custom-control-input" disabled id="radio-{{$question->id}}-4" name="answer[{{$question->id}}]" value="4">
